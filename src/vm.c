...
mrb_funcall_with_block(mrb_state *mrb, mrb_value self, mrb_sym mid, mrb_int argc, const mrb_value *argv, mrb_value blk)
{
  ...
  if (!MRB_GET_THREAD_CONTEXT(mrb)->jmp) {
    ...
    ptrdiff_t nth_ci = MRB_GET_CONTEXT(mrb)->ci - MRB_GET_CONTEXT(mrb)->cibase;
    ...
    MRB_CATCH(&c_jmp) { /* error */
      while (nth_ci < (MRB_GET_CONTEXT(mrb)->ci - MRB_GET_CONTEXT(mrb)->cibase)) {
    ...
    if (MRB_GET_CONTEXT(mrb)->stbase <= argv && argv < MRB_GET_CONTEXT(mrb)->stend) {
      voff = argv - MRB_GET_CONTEXT(mrb)->stbase;
    }
    ...
    if (voff >= 0) {
      argv = MRB_GET_CONTEXT(mrb)->stbase + voff;
    }
    ...
    CASE(OP_RETURN) {
      ...
      L_RESCUE:
        ...
        case OP_R_BREAK:
          ...
          ci = MRB_GET_CONTEXT(mrb)->ci;
          MRB_GET_CONTEXT(mrb)->stack = ci->stackent;
