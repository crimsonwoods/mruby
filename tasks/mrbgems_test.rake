      ...
      open(t.name, 'w') do |f|
        ...
        unless g.test_rbfiles.empty?
          ...
          g.test_rbfiles.count.times do |i|
            ...
            f.puts %Q[    exit(EXIT_FAILURE);]
            f.puts %Q[  }]
            f.puts %Q[  mrb_const_set(mrb2, mrb_obj_value(MRB_GET_VM(mrb2)->object_class), mrb_intern_lit(mrb2, "GEMNAME"), mrb_str_new(mrb2, "#{g.name}", #{g.name.length}));]
            ...
            unless g.test_args.empty?
              ...
              f.puts %Q[  mrb_const_set(mrb2, mrb_obj_value(MRB_GET_VM(mrb2)->object_class), mrb_intern_lit(mrb2, "TEST_ARGS"), test_args_hash); ]
            end
